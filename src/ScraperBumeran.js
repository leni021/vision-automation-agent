...
export default class ScraperBumeran extends BaseScraper {
  constructor({
    page,
    credentials,
    readDelayMinMs,
    ...
    this.baseUrl = "https://www.bumeran.com.ar";
  ...
  async login() {
    ...
    if (!this.credentials?.email || !this.credentials?.password) {
      throw new Error("Faltan credenciales de Bumeran.");
    }
    ...
    if (!opened) {
      throw new Error("No se pudo abrir la pantalla de login de Bumeran.");
    }
    ...
    if (!emailSelector || !passwordSelector) {
      throw new Error("No se pudieron completar campos de login de Bumeran.");
    }
    ...
    if (!submitSelector) {
      throw new Error("No se encontro boton de ingreso en Bumeran.");
    }
    ...
    if (bodyText.includes("datos incorrectos") || bodyText.includes("credenciales invalidas")) {
      throw new Error("Bumeran rechazo las credenciales.");
    }
  ...
  async runUiSearch(query, location) {
    ...
    if (!queryInput) {
      this.log("No se encontro el input principal de busqueda en Bumeran.");
      return false;
    }
  ...
  async searchOffers({ queries = DEFAULT_QUERIES, location = "Argentina", maxLinks = 30 } = {}) {
    ...
    const linkRules = [
      /bumeran\.com\.ar\/empleos\/[a-z0-9-]+\.html(\?.*)?$/i,
    ...
    for (const query of queries) {
      ...
      links = links.filter((entry) => {
        ...
        return /bumeran\.com\.ar\/empleos\/[a-z0-9-]+\.html/i.test(String(url ?? "")) &&
